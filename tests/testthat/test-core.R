test_that("describe data frames and imputers", {
  ...
  expect_equal(d$n, 3)
  expect_equal(d$p, 2)
  expect_s3_class(d$variable_summary, "data.table")
  expect_s3_class(d$row_summary, "data.table")
  expect_s3_class(d$pattern, "data.table")
...
test_that("ampute mechanisms work", {
  ...
  expect_s3_class(mcar$data, "data.table")
  expect_s3_class(mcar$mask_added, "data.table")
  expect_s3_class(summary(mcar), "data.table")
...
test_that("naive imputation works", {
  ...
  expect_s3_class(i1$data, "data.table")
  expect_s3_class(i1$imputations[[1]], "data.table")
  expect_s3_class(i1$imputed_cells, "data.table")
  ...
  expect_s3_class(complete(i1), "data.table")
  expect_s3_class(complete(i3, "long"), "data.table")
  expect_true(all(vapply(complete(i3, "all"), inherits, logical(1), what = "data.table")))
...
test_that("internal chained equation imputation uses fit/predict steps", {
  ...
  s <- summary(imp)
  expect_s3_class(s$overview, "data.table")
  expect_s3_class(s$variables, "data.table")
  ...
  expect_s3_class(imp$diagnostics$trace, "data.table")
...
test_that("imputer learners expose a standard fit/predict contract", {
  ...
  expect_s3_class(imputer_registry(), "data.table")
...
test_that("densemlp imputer fits/predicts through the standard chained loop", {
  skip_if_not_installed("densemlp")
  dat <- data.frame(
    a = c(1, 2, NA, 4, 5, 6, NA, 8, 9, 10),
    b = c(2, 1, 3, 4, 5, 6, 7, 8, 9, 10),
    c = factor(c("x", "x", "y", NA, "y", "x", "y", "x", NA, "y"))
  )
  imp <- impute(dat, m = 1, imputer = "densemlp", maxit = 1, seed = 1,
               epochs = 5, verbose = FALSE)
  expect_s3_class(imp, "mimar_imputation")
  expect_false(anyNA(complete(imp)))
  expect_true(all(imp$variable_methods[c("a", "c")] == "densemlp"))
  expect_equal(imputer("densemlp")$package, "densemlp")
})

test_that("missknn imputer bypasses the chained loop as a whole-table strategy", {
  skip_if_not_installed("missknn")
  dat <- data.frame(
    a = c(1, 2, NA, 4, 5, 6, 7, NA, 9, 10),
    b = c(2, NA, 3, 4, 6, NA, 8, 9, 10, 11),
    g = factor(c("x", "x", "y", NA, "y", "x", "y", "x", NA, "y"))
  )
  imp <- impute(dat, m = 2, imputer = "missknn", k = 3, seed = 1)
  expect_s3_class(imp, "mimar_imputation")
  expect_length(imp$imputations, 2)
  expect_false(anyNA(complete(imp)))
  expect_equal(imp$diagnostics$strategy, "missknn")
  expect_true(all(imp$variable_methods == "missknn"))
  expect_equal(imputer("missknn")$package, "missknn")
})

test_that("progress argument is validated and threaded through fmap", {
  dat <- data.frame(
    a = c(1, 2, NA, 4, 5, NA, 7, 8),
    b = c(2, NA, 4, 5, NA, 7, 8, 9)
  )
  expect_s3_class(impute(dat, m = 1, imputer = "naive", maxit = 1, progress = TRUE), "mimar_imputation")
  expect_s3_class(impute(dat, m = 1, imputer = "naive", maxit = 1, progress = FALSE), "mimar_imputation")
  expect_error(impute(dat, imputer = "naive", progress = NA), "`progress`")
})
...
test_that("evaluate uses amputation truth", {
  ...
  expect_s3_class(e$summary, "data.table")
  expect_s3_class(e$distribution, "data.table")
  expect_s3_class(e$recovery, "data.table")
  expect_s3_class(e$recovery_by_imputation, "data.table")
  expect_s3_class(e$variability, "data.table")
...
test_that("pool estimates and metrics", {
  ...
  expect_s3_class(ps$pooled, "data.table")
  ...
  expect_s3_class(p$pooled, "data.table")
  expect_s3_class(p$data, "data.table")
  ...
  expect_s3_class(pm$pooled, "data.table")
