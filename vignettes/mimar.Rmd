...
conditional distribution implied by the imputation model. No single plot can
prove that assumption, but several simple graphics expose common failures:
...
store by default but rich enough to support convergence trace plots.

```{r}
head(i_naive$diagnostics$trace)
```
...
signal lack of convergence, feedback between derived variables, or a poor
...
meaningful stochastic updates. Trace plots are most informative for stochastic
methods such as `pmm`, `norm`, `logreg`, `polyreg`, and learner-backed
imputers.

```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "trace", statistic = "mean")
```
...
imputation contributes its own imputed curve. The marginal distributions do not
have to be identical under MAR, because missingness may depend on observed
covariates. Large differences should still be investigated because they may
represent either a real conditional difference or an imputation-model problem.

```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "density", variable = "bmi")
```
...
```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "boxplot", variable = "bmi")
```
...
```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "strip", variable = "bmi")
```
...
```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "xy", formula = bmi ~ age | sex)
```
...
```{r, fig.width=7, fig.height=4}
plot(i_naive, type = "proportion", variable = "group")
plot(i_naive, type = "proportion", formula = group ~ sex)
```
