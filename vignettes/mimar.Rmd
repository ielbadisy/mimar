...
No external modelling framework is required for the default methods; optional
learner-backed imputers load their backend packages only when requested.
...
`mimar` ships with a set of native imputers and exposes hooks for optional
learner-backed methods. The registry documents each available method, whether
it is implemented natively or through an optional package dependency, and
which target types it supports.
...
FAMD-assisted coordinates when `missMDA` is available [@josse2016missmda],
then applying donor matching in that space.
...
When the required backend packages are installed, learner-backed methods are
...
rules [@rubin1987] accomplishes this.

Let $Q_k$ be the estimate and $U_k$ the complete-data variance estimate from
completed dataset $k$. The pooled point estimate and between-imputation
variance are:
...
`pool()` accepts a data frame of per-imputation results in tidy format and
returns pooled estimates, total standard errors, confidence intervals, and
