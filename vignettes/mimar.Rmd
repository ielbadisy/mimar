...
`rf`, `ranger`, `rpart`, `nbayes`, `svm`, `bart`, `glmnet`, `fastgbm`,
`densemlp`, and `xgboost` treat the imputation of each variable as a supervised
learning problem. The learner is fitted on observed rows and predicts (or draws from)
...
by `mimar`. The `ranger`, `glmnet`, and `xgboost` backends follow
@wright2017ranger, @friedman2010glmnet, and @chen2016xgboost, respectively;
`fastgbm` is a native gradient-boosting implementation in the sense of
@friedman2001gbm.
...
## Pooling Regression Models: `pool_coxph()` and `pool_glm()`

`pool()` treats the analyst's fitted models as an unstructured source of
scalar quantities: term names, estimates, and standard errors must be
extracted and reshaped into the tidy adapter shown above. `pool_coxph()` and
`pool_glm()` skip that step for the two most common regression settings in
applied work. Each takes a list of models fitted directly on
`complete(imp, "all")`, one model per completed data set, and returns a
pooled coefficient table formatted the way `summary.coxph()` and
`summary.glm()` already format single-fit output, so the pooled result reads
like the familiar unpooled one plus a `df` column for the pooled
(Barnard-Rubin) degrees of freedom.

```{r}
imp <- impute(mtcars, m = 5, imputer = "pmm", seed = 1)

glm_fits <- lapply(complete(imp, "all"), function(d) {
...
The coefficient table's `Estimate` and `Std. Error` columns are the usual
Rubin pooled mean and total-variance standard error from `pool()`. The `df`
column is not the classic Rubin (1987) degrees of freedom, which can grow
implausibly large when between-imputation variance is small relative to
within-imputation variance; instead it uses the Barnard and Rubin (1999)
correction, bounded by the complete-data residual degrees of freedom
(`df.residual()` of a single fit). This matches `mice::pool()`'s default
behavior for `glm` models.

`pool_coxph()` follows the same pattern for `coxph` models, adding a second
table of exponentiated coefficients (hazard ratios) with a pooled confidence
interval, mirroring `summary.coxph()`'s two-table layout:

```{r, eval = requireNamespace("survival", quietly = TRUE)}
library(survival)

cox_fits <- lapply(complete(imp, "all"), function(d) {
  d$hi_wt <- as.integer(d$wt > median(d$wt))
  coxph(Surv(qsec, am) ~ mpg + hi_wt, data = d)
})

pool_coxph(cox_fits)
```

Because a Cox partial likelihood has no residual degrees of freedom in the
`glm` sense, `pool_coxph()` uses `nevent - p` (events minus estimated
parameters) as the complete-data df for the Barnard-Rubin correction, since
the information in a partial likelihood scales with the number of events
rather than the number of rows. Both `pool_coxph()` and `pool_glm()` require
every model in the list to share the same coefficient names in the same
order, i.e., the same formula fitted on each completed data set.

Three further helpers extend the same pattern to other common regression
settings: `pool_lm()` for ordinary linear models (`summary.lm()`-formatted
output), `pool_survreg()` for parametric accelerated-failure-time survival
models (`summary.survreg()`-formatted output, pooling the regression
coefficients only, not the distribution's scale/shape parameter), and
`pool_clogit()` for conditional logistic regression on matched sets
(`survival::clogit()` fits a stratified Cox model internally, so
`pool_clogit()` pools and formats identically to `pool_coxph()`, including
the `nevent - p` degrees-of-freedom convention). All three, together with
`pool_coxph()` and `pool_glm()`, were cross-validated against
`mice::pool()` on matched examples and reproduce its pooled estimates,
standard errors, degrees of freedom, and p-values to numerical precision.
